...
// The functions below originate from Fast R-CNN
...
__device__ inline bool devIoU(scalar_t const * const a, scalar_t const * const b, const float threshold) {
  ...
  const int end_a = start_a + a[4] - 1 + 0.5 - ((a[4] - 1) < 0); // - (x<0) trick to adjust for negative numbers
...
std::vector<at::Tensor> nms_cuda_forward(
  ...
  TORCH_CHECK(col_blocks < MAX_COL_BLOCKS, 
