...
import torch
import numbers
from collections.abc import Iterable  # Thay collections bằng collections.abc
...
class Resize(object):
    def __init__(self, size, cfg=None):
        assert (isinstance(size, Iterable) and len(size) == 2)  # Thay collections.Iterable bằng Iterable
        self.size = size
